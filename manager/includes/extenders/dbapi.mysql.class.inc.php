...
class DBAPI {
   ...
   function connect($host = '', $dbase = '', $uid = '', $pwd = '', $persist = 0) {
      ...
      if (!$this->conn = mysqli_connect($host, $uid, $pwd, $dbase, defined('DBPORT')?DBPORT:3306)) {
         $modx->messageQuit("Failed to create the database connection!");
         exit;
      } else {
         $dbase = str_replace('`', '', $dbase); // remove the `` chars
         if (!@ mysqli_select_db($this->conn, $dbase)) {
            $modx->messageQuit("Failed to select the database '" . $dbase . "'!");
            exit;
         }
         @mysqli_query($this->conn, "{$connection_method} {$charset}");
...
//      }
   ...
   function getRecordCount($ds) {
      return is_object($ds) ? mysqli_num_rows($ds) : 0;
   }
   ...
   function getColumn($name, $dsq) {
    ...
    function getColumnsCount($dsq) {
        return mysqli_num_fields($dsq);
    }


    function getColumnName($dsq, $index) {
        $fields = $this->getColumnNames($dsq);
        return $fields[$index];
    }
   ...
   function getColumnNames($dsq) {
      ...
      if ($dsq) {
         $names = array ();
          foreach($dsq->fetch_fields() as $field) {
              $names[] = $field->name;
          }
//         $limit = mysqli_num_fields($dsq);
//         for ($i = 0; $i < $limit; $i++) {
