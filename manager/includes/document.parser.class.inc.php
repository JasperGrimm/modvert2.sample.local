                ...
                if ($docObj['privateweb'] && isset ($docObj['__MODxDocGroups__'])) {
                    ...
                    if (!$pass) {
                        if ($this->config['unauthorized_page']) {
                            ...
                            if ($secrs)
                                $seclimit= mysqli_num_rows($secrs);
                        }
                        if ($seclimit > 0) {
...
//                header('HTTP/1.0 404 Not Found');
    ...
    function rewriteUrls($documentSource) {
        ...
        if ($this->config['friendly_urls'] == 1) {
            ...
            $in= '!\[\~([0-9]+)\~\]!is'; // Use preg_replace with /e to make it evaluate PHP
            ...
            $documentSource= preg_replace_callback(
                $in,
                function($m) use ($isfriendly, $aliases, $pref, $suff, $self){
                    $thealias = $aliases[$m[1]];
                    $found_friendlyurl= $self->makeFriendlyURL($pref,$suff,$thealias);
                    $not_found_friendlyurl= $self->makeFriendlyURL($pref, $suff,"'" . $m[1] . "'");
...
//            $documentSource= preg_replace($in, $out, $documentSource);
            $documentSource= preg_replace_callback(
                $in,
                function($m) { return "index.php?id=" . '\1'; },
                $documentSource
            );
        }
        return $documentSource;
    ...
    function dbExtConnect($host, $user, $pass, $dbase) {
        ...
        if (@ !$this->rs= mysqli_connect($host, $user, $pass)) {
