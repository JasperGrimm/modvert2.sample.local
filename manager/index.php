...
include_once "./multilang.document.parser.class.php";
$modx = new MLDocumentParser;
$modx->loadExtension("ManagerAPI");
$modx->getSettings();
$etomite = &$modx; // for backward compatibility

// connect to the database
if(@!$modxDBConn = mysqli_connect($database_server, $database_user, $database_password, str_replace('`', '', $dbase))) {
    die("<h2>Failed to create the database connection!</h2>. Please run the MODx <a href='../install'>install utility</a>");
} else {
    mysqli_select_db($modxDBConn, str_replace('`', '', $dbase));
    @mysqli_query($modxDBConn, "{$database_connection_method} {$database_connection_charset}");
}
