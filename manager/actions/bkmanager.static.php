...
class Mysqldumper {
  ...
  function getDBconnectionMethod() { return $this->_dbconnectionmethod; }
	...
	function createDump($callBack) {
		...
		$resource = mysqli_connect($this->getHost(), $this->getDBuser(), $this->getDBpassword());
		mysqli_select_db($resource, $this->getDBname());
		$database_connection_method = $this->getDBconnectionMethod(); 
		$database_connection_charset = $this->getDBcharset();
		@mysqli_query($resource, "{$database_connection_method} {$database_connection_charset}");
