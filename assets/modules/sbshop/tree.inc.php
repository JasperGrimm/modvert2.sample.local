...
function SBShopGetFolders($parent) {
	...
	$sql = "SELECT * FROM " . $modx->getFullTableName('sbshop_categories') . " WHERE `category_parent` = '{$parent}'";
	//var_dump($sql);
...
function SBShopGetFiles($folder) {
	...
	$sql = "SELECT * FROM " . $modx->getFullTableName('sbshop_products') . " WHERE `product_category` = '{$folder}'";
	...
	while($file = $modx->fetchRow($files)) {
		$return[$file['product_id']] = array(
			...
			'image' => 'assets/modules/3b-modules/fileorganizer/images/tree/package.png'
		);
	}
	return $return;
}
